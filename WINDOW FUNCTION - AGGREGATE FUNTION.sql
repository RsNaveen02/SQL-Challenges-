--Day 20 
	--Q1. Calculate running total of patients admitted by week for each service.
			SELECT service,week,patients_admitted,
			SUM(patients_admitted) OVER (
			PARTITION BY service
			ORDER BY week
			
			) as running_total
			FROM services_weekly

	--Q2. Find the moving average of patient satisfaction over 4-week periods.
			SELECT patient_satisfaction,week,
			ROUND(AVG(patient_satisfaction) OVER(
			ORDER BY week
			ROWS BETWEEN 3 PRECEDING AND CURRENT ROW
			),2)as  MOVING_AVG_week
			FROM services_weekly
	
	--Q3. Show cumulative patient refusals by week across all services.
			 SELECT week,service,patients_refused,
			 SUM(patients_refused) OVER (
				PARTITION BY service
        		ORDER BY week
        		ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
			 ) as cumulative_patient 
		  	FROM services_weekly
			  
		--Create a trend analysis showing for each service and week: week number, patients_admitted, 
		--running total of patients admitted (cumulative),
		-- 3-week moving average of patient satisfaction (current week and 2 prior weeks), 
		--and the difference between current week admissions and the service average. 
		--Filter for weeks 10-20 only.


			SELECT patients_admitted,week,service,
			ROUND(SUM(patients_admitted) OVER(
			ORDER BY week
			ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
			),2)as  Running_total,
			ROUND(AVG(patient_satisfaction) OVER(
			ORDER BY week
			ROWS BETWEEN 2 PRECEDING AND CURRENT ROW
			),2) AS average_of_patient_satisfaction,
			ROUND(patients_admitted - AVG(patient_satisfaction) OVER(
			PARTITION BY service ),2)as DIFFERENCE 
			FROM services_weekly
			WHERE week BETWEEN 10 AND 20
			ORDER BY week,service;
 